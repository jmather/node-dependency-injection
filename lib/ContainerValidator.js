...
export default class ContainerValidator {
  ...
  _checkKeyedGroupsWithNoDefault (result) {
    ...
    for (const [id, definition] of this._container.definitions) {
      if (!definition.keyedGroup) {
        continue
      }
      if (!keyedGroups.has(definition.keyedGroup)) {
        keyedGroups.set(definition.keyedGroup, [])
      }
      keyedGroups.get(definition.keyedGroup).push({ id, definition })
    }

    for (const [groupName, services] of keyedGroups) {
      if (services.length > 1) {
        const hasDefault = services.some(
          (s) => s.definition.keyedDefault === true
        )
        if (!hasDefault) {
          result.addInfo(
            ...
            '',
            `keyed:${groupName}`,
            `Keyed group '${groupName}' has ${services.length} services but none is marked as default`
          )
        }
      }
    }
